                                ...
                                memc_read_or_write read_or_write)
{
  ...
  */
  ...
  FD_SET(ptr->fd, &fds);
  tv.tv_sec = 0;
  tv.tv_usec = ptr->root->poll_timeout;
  if (read_or_write == MEM_WRITE) /* write */
    error = select(ptr->fd + 1, NULL, &fds, NULL, &tv);
  else
    error = select(ptr->fd + 1, &fds, NULL, NULL, &tv);
#endif
  ...
  while (write_length)
  {
    ...
    else
    {
      ...
      if ((sent_length= write(ptr->fd, local_write_ptr, 
                              write_length)) == -1)
#else
      if ((ssize_t)(sent_length= send(ptr->fd, local_write_ptr, 
                                       write_length, 0)) == -1)
#endif
      {
#ifdef _WIN32
